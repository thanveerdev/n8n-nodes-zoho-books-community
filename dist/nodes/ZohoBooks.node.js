...
function applyExtraFields(body, extra = []) {
...
function applyContactPersonFromExtra(body, extra = []) {
    ...
    const contactPersons = body.contact_persons || [];
    const contactPerson = {};
    let hasContactPersonData = false;
    for (const field of extra) {
        if (!field.name)
            continue;
        if (contactPersonKeys.has(field.name)) {
            ...
            if (field.name === 'is_primary_contact') {
                ...
                contactPerson[field.name] = field.value;
            }
        }
    }
    ...
    return extra.filter((field) => !contactPersonKeys.has(field.name));
}
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'organization') {
                    ...
                    const operation = this.getNodeParameter('operation', i);
                    const organizationId = this.getNodeParameter('organizationIdContact', i);
                    if (operation === 'list') {
                        ...
                        if (additionalFields.perPage)
                            qs.per_page = additionalFields.perPage;
                        if (additionalFields.searchText)
                            qs.search_text = additionalFields.searchText;
                        const response = await zohoBooksApiRequest.call(this, 'GET', '/contacts', {}, qs);
                    ...
                    if (operation === 'get') {
                        ...
                        const response = await zohoBooksApiRequest.call(this, 'GET', `/contacts/${contactId}`, {}, qs);
                        const contact = response.contact || response;
                        returnData.push({ json: contact });
                    }
                    if (operation === 'create') {
                        const rawBody = this.getNodeParameter('bodyContact', i);
                        const body = parseJsonParameter(rawBody, 'Body', this);
                        const extraWrapper = this.getNodeParameter('extraFieldsContact', i, {});
                        const extra = (extraWrapper.field || []);
                        const mappedWrapper = this.getNodeParameter('mappedFieldsContact', i, {});
                        const mappedFields = (mappedWrapper.field || []);
                        ...
                        const customFieldsWrapper = this.getNodeParameter('customFieldsContact', i, {});
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'POST', '/contacts', body, qs);
                    ...
                    if (operation === 'update') {
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'PUT', `/contacts/${contactId}`, body, qs);
                        returnData.push({ json: response });
                    }
                    if (operation === 'delete') {
                        const contactId = this.getNodeParameter('contactId', i);
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'DELETE', `/contacts/${contactId}`, {}, qs);
                        returnData.push({ json: response });
                    }
                }
                else if (resource === 'invoice') {
                    const operation = this.getNodeParameter('operation', i);
                    const organizationId = this.getNodeParameter('organizationIdInvoice', i);
                    if (operation === 'list') {
                        const additionalFields = this.getNodeParameter('additionalFieldsInvoice', i, {});
                        const qs = {
                            organization_id: organizationId
                        };
                        if (additionalFields.page)
                        ...
                        const response = await zohoBooksApiRequest.call(this, 'GET', '/invoices', {}, qs);
                    ...
                    if (operation === 'get') {
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'GET', `/invoices/${invoiceId}`, {}, qs);
                        const invoice = response.invoice || response;
                        returnData.push({ json: invoice });
                    }
                    if (operation === 'create') {
                        const rawBody = this.getNodeParameter('bodyInvoice', i);
                        const body = parseJsonParameter(rawBody, 'Body', this);
                        const extraWrapper = this.getNodeParameter('extraFieldsInvoice', i, {});
                        const extra = (extraWrapper.field || []);
                        const mappedWrapper = this.getNodeParameter('mappedFieldsInvoice', i, {});
                        const mappedFields = (mappedWrapper.field || []);
                        const mappedAsExtra = mappedFields.map((f) => ({
                            name: f.fieldKey,
                            value: f.value
                        ...
                        const customFieldsWrapper = this.getNodeParameter('customFieldsInvoice', i, {});
                        const customFields = (customFieldsWrapper.customField ||
                            []);
                        if (customFields.length) {
                            body.custom_fields = customFields.map((f) => ({
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'POST', '/invoices', body, qs);
                        returnData.push({ json: response });
                    }
                    if (operation === 'update') {
                        const invoiceId = this.getNodeParameter('invoiceId', i);
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'PUT', `/invoices/${invoiceId}`, body, qs);
                        returnData.push({ json: response });
                    }
                    if (operation === 'delete') {
                        const invoiceId = this.getNodeParameter('invoiceId', i);
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'DELETE', `/invoices/${invoiceId}`, {}, qs);
                        returnData.push({ json: response });
                    }
                }
                else if (resource === 'payment') {
                    const operation = this.getNodeParameter('operation', i);
                    const organizationId = this.getNodeParameter('organizationIdPayment', i);
                    if (operation === 'list') {
                        const additionalFields = this.getNodeParameter('additionalFieldsPayment', i, {});
                        const qs = {
                            organization_id: organizationId
                        };
                        if (additionalFields.page)
                        ...
                        const response = await zohoBooksApiRequest.call(this, 'GET', '/customerpayments', {}, qs);
                    ...
                    if (operation === 'get') {
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'GET', `/customerpayments/${paymentId}`, {}, qs);
                        const payment = response.payment || response;
                        returnData.push({ json: payment });
                    }
                    if (operation === 'create') {
                        const rawBody = this.getNodeParameter('bodyPayment', i);
                        const body = parseJsonParameter(rawBody, 'Body', this);
                        const extraWrapper = this.getNodeParameter('extraFieldsPayment', i, {});
                        const extra = (extraWrapper.field || []);
                        const mappedWrapper = this.getNodeParameter('mappedFieldsPayment', i, {});
                        const mappedFields = (mappedWrapper.field || []);
                        const mappedAsExtra = mappedFields.map((f) => ({
                            name: f.fieldKey,
                            value: f.value
                        ...
                        extra.push(...mappedAsExtra);
                        applyExtraFields(body, extra);
                        const customFieldsWrapper = this.getNodeParameter('customFieldsPayment', i, {});
                        const customFields = (customFieldsWrapper.customField ||
                            []);
                        if (customFields.length) {
                            body.custom_fields = customFields.map((f) => ({
                        ...
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'POST', '/customerpayments', body, qs);
                        returnData.push({ json: response });
                    }
                    if (operation === 'delete') {
                        const paymentId = this.getNodeParameter('paymentId', i);
                        const qs = { organization_id: organizationId };
                        const response = await zohoBooksApiRequest.call(this, 'DELETE', `/customerpayments/${paymentId}`, {}, qs);
                        returnData.push({ json: response });
                    }
                }
